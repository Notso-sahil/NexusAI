...
export function registerBuiltinSpecs() {
    ...
    schema: [
      {
        ...
        help: 'Target URL with template variable support {var}',
        default: '',
      },
    ],
    defaults: { url: '' },
    ...
    schema: [
      {
        ...
        help: 'Element selector or target locator',
      },
      {
        key: 'before',
        label: 'Pre-execution',
  ...
  registerNodeSpec({
    ...
    version: 1,
    display: { label: 'Drag & Drop', iconClass: 'icon-drag', category: 'Actions' },
    ports: { inputs: 1, outputs: [{ label: 'default' }] },
    schema: [
    ...
    schema: [
      {
        ...
        type: 'json',
        help: 'e.g. {"exists":"#id"} or {"visible":".btn"}',
      },
      {
        key: 'failStrategy',
  ...
  registerNodeSpec({
    ...
    schema: [
      ...
      { key: 'formData', label: 'FormData (JSON)', type: 'json' },
      { key: 'saveAs', label: 'Save As Variable', type: 'string' },
      { key: 'assign', label: 'Assign (JSON)', type: 'json' },
    ],
    defaults: { method: 'GET' },
  });
  ...
  registerNodeSpec({
    ...
    version: 1,
    display: { label: 'Foreach Loop', iconClass: 'icon-foreach', category: 'Logic' },
    ports: { inputs: 1, outputs: [{ label: 'default' }] },
    schema: [
      ...
      { key: 'subflowId', label: 'Subflow ID', type: 'string', required: true },
      {
        ...
        type: 'number',
        default: 1,
        help: 'Concurrent subflow execution (shallow-copied variables)',
      },
    ],
  ...
  registerNodeSpec({
    ...
    schema: [
      ...
      { key: 'description', label: 'Description', type: 'string' },
      {
        ...
        type: 'object',
        fields: [
          { key: 'manual', label: 'Manual Trigger', type: 'boolean', default: true },
          { key: 'url', label: 'URL Trigger', type: 'boolean', default: false },
          { key: 'contextMenu', label: 'Context Menu', type: 'boolean', default: false },
          { key: 'command', label: 'Shortcut Key', type: 'boolean', default: false },
