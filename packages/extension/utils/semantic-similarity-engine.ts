...
async function getCachedModelData(modelUrl: string): Promise<ArrayBuffer> {
  ...
  try {
    ...
    // 3. Fetch data and persist to cache
    ...
    // Purge incomplete cache entry on error
...
export class SemanticSimilarityEngineProxy {
  ...
  private async sendMessageToOffscreen(message: any, maxRetries: number = 3): Promise<any> {
    // Ensure offscreen document is initialized
    await this.offscreenManager.ensureOffscreenDocument();
...
export class SemanticSimilarityEngine {
  ...
  private _setupWorker(): void {
    ...
    // Strategy 1: Chrome extension URL (recommended for production)
    try {
  ...
  /**
   * Internal initialization with progress callback
   */
  private async _doInitializeWithProgress(
    ...
    try {
      ...
      // Detect execution context and select mode
      const workerSupported = this.isWorkerSupported();
      const inOffscreenDocument = this.isInOffscreenDocument();

      // Avoid recursion: force direct Worker mode when inside offscreen document
      ...
      if (this.useOffscreen) {
        // Offscreen mode: delegate to offscreen document
        ...
        const configToSend = {
          ...
          useLocalFiles: Boolean(this.config.useLocalFiles), // Cast to boolean
        ...
        // Direct Worker mode: provide live progress tracking
    ...
    try {
      ...
      if (this.useOffscreen) {
        ...
        const response = await chrome.runtime.sendMessage({
          target: 'offscreen',
          type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_INIT,
          config: serializedConfig, // Use original config without mutation
        });
        ...
        if (this.config.useLocalFiles) {
          // For WXT, public directory assets reside at runtime root
          // Use model identifier directly; transformers.js prepends /models/
        ...
        // Explicitly configure tokenizer for models not requiring token_type_ids
        ...
        // Attempt SIMD acceleration initialization
        try {
  ...
  /**
   * Direct Worker initialization with progress callback
   */
  private async _initializeDirectWorkerWithProgress(
    ...
    // Use transformers.js progress callback
    ...
    try {
      ...
      // Fallback to standard initialization if progress is unsupported
      console.log(
  ...
  public async warmupModel(): Promise<void> {
    ...
    // Representative warm-up prompts covering varying token lengths
    const warmupTexts = [
      // Short prompt
      'Hello',
      'Hello',
      ...
      'Hello world, this is an embedding tensor verification test.',
    ...
    try {
      // Progressive warm-up: single query then batch
  ...
  private async _tokenizeText(text: string | string[]): Promise<TokenizedOutput> {
    ...
    // Attempt cache lookup for single text
    if (typeof text === 'string') {
      ...
      // Set return_token_type_ids to false when unneeded
      if (!this.config.requiresTokenTypeIds) {
  ...
  private _extractEmbeddingFromWorkerOutput(
    ...
    // Optimization: use Float32Array directly
    ...
    // Acquire embedding array from buffer pool
  ...
  public async getEmbedding(
    ...
    // Delegate to offscreen document when in offscreen mode
    if (this.useOffscreen) {
      const response = await chrome.runtime.sendMessage({
        target: 'offscreen',
        type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_COMPUTE,
  ...
  public async getEmbeddingsBatch(
    ...
    if (this.useOffscreen) {
      ...
      // Return directly if all entries are cached
      ...
      // Insert results into corresponding positions and cache
  ...
  public async computeSimilarityMatrix(
    ...
    // Use SIMD-accelerated matrix operations when available
  ...
  public cosineSimilarity(vecA: Float32Array, vecB: Float32Array): number {
    ...
    if (this.useSIMD && this.simdMath) {
      try {
        // SIMD version is async; keeping sync fallback for compatibility
        // Fallback to JS version or async implementation
  ...
  public validateInput(text1: string, text2: string | 'valid_dummy'): void {
    ...
    if (
      ...
      console.warn('Input text may be truncated by tokenizer.');
    }
  }
  ...
  // Get Worker telemetry statistics
