/**
 * Output Sanitizer - redaction and size limit utilities
 *
 * Provides security processing for script execution outputs:
 * 1. Sensitive credential redaction (cookie/token/password)
 * 2. Output length constraint (default 50KB)
...
// Sensitive key identifiers targeted for redaction
// Standard sensitive key definitions
...
/**
 * Sanitize and constrain output values
 */
...
/**
 * Redact sensitive credential patterns from string
 * Redaction pattern matching with Base64/Hex/cookie detection
 */
export function sanitizeText(text: string): { text: string; redacted: boolean } {
  ...
  // 6. Embedded Base64 tokens
  replace(/\b[A-Za-z0-9+/]{40,}={0,2}\b/g, '<redacted_base64>');

  // 7. Embedded long hex credentials
...
/**
 * Check if string resembles query string
 */
...
/**
 * Check if string resembles cookie string
 */
...
function truncateTextBytes(
  ...
  // Binary search for truncation boundary
  let lo = 0;
  let hi = text.length;
