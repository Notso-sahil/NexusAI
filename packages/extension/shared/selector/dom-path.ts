/**
 * DOM Path - Calculation and positioning
 *
...
/**
 * DOM path: child index array from root to target element
 *
 * @example
 * ```
 * [0, 2, 1] represents:
...
/**
 * Calculate element path in DOM tree
 *
 * target element（Document  ShadowRoot），
 * recording index in parent children at each level.
 *
 * @example
 * ```ts
...
export function computeDomPath(element: Element): DomPath {
  ...
  while (current) {
    ...
    // Check if direct child of ShadowRoot or Document
    ...
    // Reached root node, terminate traversal
    break;
  }
 ...
 * @param path - DOM path
 * @returns Found element, or null if path invalid
 *
 * @example
 * ```ts
 ...
 *  DOM path
 *
 * @returns Result indicating equality and common prefix length
 *
 * @example
 * ```ts
