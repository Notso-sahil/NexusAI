/**
 * Selector Locator - Element positioning
 * Locate DOM elements using candidate selector list
 */
  ...
  async locate(
    ...
    if (typeof target.selector === 'string' && target.selector.trim()) {
      ...
      if (ensured) {
        ...
        // Fingerprint validation: skip and try next candidate if mismatched
        const fingerprintOk =
          !fingerprintToVerify ||
        ...
        // Fingerprint mismatch, try candidate selector
      }
    }
