 ...
 *
 * Maintains a rolling buffer per tab to continuously collect console events.
 * Automatically clears buffer when tab navigates to a new domain to prevent cross-site log pollution.
 */
...
/**
 * Extract safe preview data from CDP RemoteObject, discarding objectId to prevent memory leaks
 */
function extractArgPreview(arg: unknown): unknown {
  const a = arg as Record<string, unknown>;
  if (!a || typeof a !== 'object') return arg;

  // Retain safe fields only, discard objectId
...
class ConsoleBuffer {
  ...
  /**
   * Check if target tab has active buffer mode capture
   */
  ...
  /**
   * Ensure buffer capture is started for target tab
   */
  ...
  /**
   * Read buffer contents for target tab
   */
  ...
  private handleDebuggerEvent(
    ...
    if (method === 'Runtime.consoleAPICalled' && p) {
      ...
      state.messages.push({
        ...
        // Store safe preview data only to prevent memory leaks
