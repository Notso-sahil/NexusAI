...
interface ConsoleToolParams {
  ...
  clear?: boolean; // Clear buffer before reading
  clearAfterRead?: boolean; // Clear buffer after reading
  pattern?: string;
  onlyErrors?: boolean;
  limit?: number;
}
  ...
  async execute(args: ConsoleToolParams): Promise<ToolResult> {
    ...
    try {
      ...
      if (resolvedMode === 'buffer') {
        ...
        // Handle clear-after-read request to avoid duplicate reads
  ...
  private async captureConsoleMessages(
    ...
    try {
      ...
      try {
        ...
        // If buffer mode is using this tab, do not detach Runtime/Log domains
