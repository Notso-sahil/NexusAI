    ...
    <div v-show="activeTab === 'element-markers'" class="element-markers-content">
      <div class="px-4 py-4">
        ...
        <div class="em-toolbar">
          <div class="em-search-wrapper">
            ...
            <input
              ...
              placeholder="Search markers, selectors, domains..."
              type="text"
            />
            <button
              v-if="markerSearch"
        ...
        <div v-if="markerEditorOpen" class="em-modal-overlay" @click.self="closeMarkerEditor">
          <div class="em-modal">
            <div class="em-modal-header">
              <h3 class="em-modal-title">{{ editingMarkerId ? 'Edit Marker' : 'New Marker' }}</h3>
            ...
            <form @submit.prevent="saveMarker" class="em-form">
              <div class="em-form-row">
                <div class="em-field">
                  <label class="em-field-label">Marker Name</label>
                  <input
                    v-model="markerForm.name"
                    class="em-input"
                    placeholder="e.g., Primary Submit Button"
                    required
                  />
                </div>
              </div>
              ...
              <div class="em-form-row em-form-row-multi">
                ...
                <div class="em-field">
                  <label class="em-field-label">Target Scope</label>
                  <div class="em-select-wrapper">
                    <select v-model="markerForm.matchType" class="em-select">
                      ...
                      <option value="host">Entire Host</option>
                    </select>
                  </div>
                </div>
              </div>
              ...
              <div class="em-form-row">
                <div class="em-field">
                  <label class="em-field-label">Target Selector</label>
                  <textarea
          ...
          <div class="em-stats-bar">
            <span class="em-stats-text">
              <template v-if="markerSearch">
                ...
                {{ markers.length }} across {{ groupedMarkers.length }} hosts)
              </template>
              <template v-else>
                Total: <strong>{{ markers.length }}</strong> markers across
        ...
        <div v-else-if="markers.length > 0 && filteredMarkers.length === 0" class="em-empty">
          <p>No matching element markers found</p>
        ...
        <div v-else class="em-empty">
          <p>No element markers recorded yet</p>
...
function createTrigger() {
  ...
  alert('V3 Trigger automation pipeline is being finalized');
}
...
async function run(id: string) {
  try {
    ...
    if (!result) console.warn('Workflow execution returned non-success result');
  } catch {}
}

function edit(id: string) {
  // V3 Builder not yet implemented - show message
  alert('V3 Visual Workflow Builder is launching in next release');
...
async function remove(id: string) {
  try {
    const ok = confirm('Delete this workflow? This action cannot be reversed.');
    if (!ok) return;
