 ...
 *
 * Note：Use bidirectional scanning strategy，from target position expanding to both sides，
 * Avoids missing nearby elements when target is in the latter half of children.
 */
 ...
 * - lockX is vertical alignment line, so display Y spacing
 * - lockY is horizontal alignment line, so display X spacing
 */
