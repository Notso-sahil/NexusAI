...
function createTextAlignIcon(value: TextAlignValue): SVGElement {
  ...
  // Text line position config: [x-start, width]
  const lineConfigs: Record<TextAlignValue, Array<[number, number]>> = {
    ...
    center: [
      [3.5, 8], // Long line center
      [5, 5], // Short line center
      [4.25, 6.5], // Medium line center
    ],
    right: [
      [3.5, 8], // Long line
      [6.5, 5], // Short line right
      [5.5, 6.5], // Medium line right
    ],
    justify: [
      [3.5, 8], // Full width
      [3.5, 8], // Full width
...
function createVerticalAlignIcon(value: VerticalAlignValue): SVGElement {
  ...
  // Content block Y coordinate based on alignment
  const blockY: Record<VerticalAlignValue, number> = {
    top: 3.5, // Top align
    middle: 5.5, // Center align
    bottom: 7.5, // Bottom align
    baseline: 6.5, // Baseline slightly offset
  };

  // Two small squares representing child elements
