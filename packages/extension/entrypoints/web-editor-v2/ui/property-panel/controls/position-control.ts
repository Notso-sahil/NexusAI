...
function createPositionIcon(position: PositionValue): SVGElement {
  ...
  switch (position) {
    ...
    case 'relative': {
      // Dashed rect shows original position, filled block shows offset position
    ...
    case 'absolute':
      // Position reference line (container edge to element)
      addPath('M3 5.5H6M8 3V6', '0.8');
      // Element block in bottom-right corner
      addBlock(6, 6, 5, 5);
      break;

    case 'fixed': {
      // Pin shape representing fixed position
...
function createFlipXIcon(): SVGElement {
  ...
  // Left mirror block (semi-transparent indicates original)
  ...
  // Right mirror block (solid indicates flipped)
...
function createFlipYIcon(): SVGElement {
  ...
  // Top mirror block (semi-transparent indicates original)
  ...
  // Bottom mirror block (solid indicates flipped)
