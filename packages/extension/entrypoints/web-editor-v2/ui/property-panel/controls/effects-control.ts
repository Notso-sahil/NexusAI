...
export function createEffectsControl(options: EffectsControlOptions): DesignControl {
  ...
  // Why WeakMap is used:
  // 1. Hidden effects are not written to CSS (enabled=false), but need to be retained within session for restoration
  // 2. WeakMap guarantees automatic garbage collection when elements are removed without manual cleanup
  // 3. Only reads inline styles (not computed), so cache only preserves user hide actions
  ...
  function isEditing(): boolean {
    // Only prevent refresh when a popover is open or a transaction is active
    // Avoid overly broad focus detection that prevents external styles from syncing
  ...
  function closePopover(opts?: { commit?: boolean; rollback?: boolean }): void {
    ...
    // Sync once after closing to ensure currentItems match real inline styles
    // Avoid inconsistency after browser normalization or value correction
    if (wasOpen && !rollback) {
      syncFromTarget(true);
    }
  }
  ...
  // Listen on document capture phase to close when clicking outside Effects control
  ...
  // Listen to keyboard events on root capture phase
