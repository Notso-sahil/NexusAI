<template>
  ...
  <div class="builder-page rr-theme" :data-theme="theme">
    <div v-if="fallbackNotice" class="notice-top">
      <span>Applied fallback recommendation: promoted {{ fallbackNotice.type }} priority</span>
    ...
    <div class="main">
      ...
      <div class="topbar rr-topbar backdrop-blur">
        <div class="left">
          ...
          <span class="tip">Visual Workflow Studio</span>
        </div>
        <div class="right">
          ...
          <label class="top-btn import" title="Import JSON">
            ...
            </svg>Import<input type="file" accept="application/json" @change="onImport" />
          </label>
          ...
          >
            ...
            </svg>Run Selected</button>
          <button class="top-btn primary" @click="runAll" title="Replay Entire Workflow">
            <svg
              width="14"
              height="14"
              viewBox="0 0 24 24"
      ...
      <div class="bottom-toolbar">
        ...
        <button class="toolbar-btn" @click="fitAll" title="Fit to Screen">
          <svg
            width="16"
            height="16"
            viewBox="0 0 24 24"
...
const title = ref('Workflow Studio');
  ...
  if (q.flowId) {
    try {
      ...
      if (flowV3) {
        ...
        pushToast(`Workflow "${q.flowId}" not found; initialized new workflow`, 'warn');
        initEmptyFlow();
      }
    } catch (e) {
...
/**
 * Persist flow to V3 RPC
 * @returns Returns FlowV3 on success, null on error
 */
async function save(): Promise<FlowV3 | null> {
  try {
    ...
    pushToast(`Failed to save workflow: ${e instanceof Error ? e.message : String(e)}`, 'error');
    return null;
  }
}
...
/**
 * Convert V2 schedule to cron expression
 * @returns cron expression or null if non-convertible
 */
function scheduleToCron(schedule: { type?: string; when?: string }): string | null {
  ...
  // V3 cron does not support 'once' triggers
  return null;
}

/**
 * Synchronize trigger node configs to V3 storage
 * @description V2 schedules are mapped to V3 cron triggers
 */
      ...
      cfg.schedules.forEach((s: any, i: number) => {
        ...
        if (!cron) {
          ...
          if (scheduleType === 'once') {
            pushToast(
              `Node ${n.id} schedule #${i + 1}: V3 does not support once triggers; skipped`,
              'warn',
            );
          } else {
            pushToast(
              `Node ${n.id} schedule #${i + 1}: Unable to convert to cron (type=${scheduleType}); skipped`,
              'warn',
            );
          }
          return;
...
async function exportFlow() {
  try {
    ...
    pushToast(`Failed to export workflow: ${e instanceof Error ? e.message : String(e)}`, 'error');
  }
}
...
async function onImport(e: Event) {
  ...
  try {
    ...
    if (!candidates.length) {
      pushToast('Failed to import: No workflow data found', 'error');
      return;
    }
    ...
    pushToast(`Failed to import workflow: ${e instanceof Error ? e.message : String(e)}`, 'error');
  } finally {
    input.value = '';
  }
}
...
async function runFromSelected() {
  ...
  try {
    ...
    pushToast(`Failed to execute workflow: ${e instanceof Error ? e.message : String(e)}`, 'error');
  }
}
