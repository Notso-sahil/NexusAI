<template>
  <div class="popup-container agent-theme" :data-agent-theme="agentTheme">
    ...
    <div v-show="currentView === 'home'" class="home-view">
      <div class="header">
        <div class="header-content">
          <h1 class="header-title">NexusAI Browser Agent</h1>
        </div>
      </div>
      <div class="content">
        ...
        <!-- Quick Directives Card -->
        <div class="section">
          <h2 class="section-title">Quick Directives</h2>
          <div class="rr-icon-buttons">
            <button
              class="rr-icon-btn rr-icon-btn-record rr-icon-btn-coming-soon has-tooltip"
              @click="startRecording"
              data-tooltip="Workflow recording in active development"
            >
            ...
            <button
              ...
              data-tooltip="Toggle Interactive Page Inspector"
            >
              <EditIcon />
            </button>
            <button
              ...
              data-tooltip="Toggle Element Coordinate Marker"
            >
              <MarkerIcon />
            </button>
          </div>
        ...
        <!-- Workspace Entry Panels -->
        <div class="section">
          <h2 class="section-title">Workspace Panels</h2>
          <div class="entry-card">
            <button class="entry-item" @click="openAgentSidepanel">
              ...
              <div class="entry-content">
                <span class="entry-title">Autonomous Agent</span>
                <span class="entry-desc">Agent directives & task execution</span>
              </div>
              <svg
                class="entry-arrow"
                viewBox="0 0 24 24"
            ...
            <button class="entry-item entry-item-coming-soon" @click="openWorkflowSidepanel">
              ...
              <div class="entry-content">
                ...
                <span class="entry-desc">Automated workflow replay</span>
              </div>
              <svg
                class="entry-arrow"
                viewBox="0 0 24 24"
            ...
            <button class="entry-item" @click="openElementMarkerSidepanel">
              ...
              <div class="entry-content">
                <span class="entry-title">Coordinate Map</span>
                <span class="entry-desc">Inspect visual element boundaries</span>
              </div>
              <svg
                class="entry-arrow"
                viewBox="0 0 24 24"
            ...
            <button class="entry-item" @click="currentView = 'local-model'">
              ...
              <div class="entry-content">
                <span class="entry-title">Neural Vector Engine</span>
                <span class="entry-desc">Transformer models & semantic cache</span>
              </div>
              <svg
                class="entry-arrow"
                viewBox="0 0 24 24"
      ...
      <div class="footer">
        ...
        <p class="footer-text">NexusAI Autonomous Browser Runtime</p>
      </div>
    </div>

    <!-- Local Model Sub-view -->
    <LocalModelPage
    ...
    <!-- Sidepanel manages workflow executions; builder opens in separate window -->
    ...
    <Transition name="toast">
      <div v-if="comingSoonToast.show" class="coming-soon-toast">
        ...
        <span>{{ comingSoonToast.feature }} is under active development.</span>
      </div>
    </Transition>
  </div>
</template>
...
// Flow editor operates in standalone window
...
// Execution history viewable in sidepanel
const startRecording = async () => {
  // Workflow recording active development
  showComingSoonToast('Workflow Recording');
  return;
...
const runFlow = async (flowId: string) => {
  try {
    ...
    if (!(res && res.success)) {
      console.warn('Workflow replay failed');
      return;
    }
    ...
    try {
      ...
      if (result && result.success === false) {
        ...
        if (failed && failed.stepId) {
          // Focus failed step in builder window
    ...
    console.error('Workflow replay failed:', e);
  }
};

// Overrides and schedules managed in sidepanel

const nativeConnectionStatus = ref<'unknown' | 'connected' | 'disconnected'>('unknown');
const isConnecting = ref(false);
const nativeServerPort = ref<number>(12307);
...
function openWorkflowSidepanel() {
  // Workflow engine active development
  showComingSoonToast('Workflow Engine');
...
async function toggleElementMarker() {
  try {
    ...
    if (!tab?.id) {
      console.warn('Unable to determine active tab');
      return;
    }

    // Dispatch element marker initialization to background worker
...
const saveSemanticEngineState = async () => {
  try {
    ...
    console.error('Failed to persist semantic engine state:', error);
  }
};
...
const checkNativeConnection = async () => {
  try {
    ...
    console.error('Failed to query native link status:', error);
...
const checkServerStatus = async () => {
  try {
    ...
    console.error('Failed to query server health:', error);
  }
};
...
const testNativeConnection = async () => {
  ...
  try {
    if (nativeConnectionStatus.value === 'connected') {
      ...
      console.log(`Attempting bridge link on port: ${nativeServerPort.value}`);
      // eslint-disable-next-line no-undef
      const response = await chrome.runtime.sendMessage({
        type: 'connectNative',
      ...
      if (response && response.success) {
        ...
        console.log('Bridge link established:', response);
        await savePortPreference(nativeServerPort.value);
      } else {
        nativeConnectionStatus.value = 'disconnected';
        console.error('Bridge link rejected:', response);
      }
    }
  } catch (error) {
...
const savePortPreference = async (port: number) => {
  try {
    ...
    console.log(`Port preference persisted: ${port}`);
  } catch (error) {
    console.error('Failed to persist port preference:', error);
  }
};
...
const saveModelState = async () => {
  try {
    ...
    console.error('Failed to persist model state:', error);
  }
};
...
const startModelStatusMonitoring = () => {
  ...
  statusMonitoringInterval = setInterval(async () => {
    try {
      ...
      console.error('Failed to query model state:', error);
    }
  }, 1000);
};
...
const switchModel = async (newModel: ModelPreset) => {
  ...
  try {
    ...
    if (response && response.success) {
      ...
      console.log(
        'Transformer model switched successfully:',
        newModel,
        'version: quantized',
        'dimension:',
...
/* Active recording pulse animation */
