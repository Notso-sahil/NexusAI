<template>
  <div class="selector">
    <div class="row">
      ...
      <button class="btn-mini" type="button" title="Inspect Element" @click="onPick">Inspect</button>
    </div>
    <div class="help">Enter CSS selector or click "Inspect" to target an element directly</div>
...
async function onPick() {
  try {
    ...
    if (!res || !res.success) {
      ...
      throw new Error(res?.error || 'Element inspection failed');
    }
    ...
    if (sel) {
      ...
      err.value = 'Could not compute a stable selector; please enter one manually';
    }
  } catch (e: any) {
