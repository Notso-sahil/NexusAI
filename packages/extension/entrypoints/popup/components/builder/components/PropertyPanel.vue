<template>
  ...
  <aside class="property-panel">
    <div v-if="node" class="panel-content">
      ...
      <div class="form-section">
        <div class="form-group">
          ...
          <input class="form-input" v-model="node.name" placeholder="Enter node label" />
        </div>
      </div>
    ...
    <div v-else class="panel-empty">
      ...
      <div class="empty-text">Select a node<br />to configure properties</div>
    </div>
  </aside>
</template>
...
const extractErrors = computed(() => {
  ...
  const errs: string[] = [];
  if (!n.config?.saveAs) errs.push('Variable name required');
  if (!n.config?.selector && !n.config?.js) errs.push('Selector or JS expression required');
  return errs;
});
