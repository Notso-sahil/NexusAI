  ...
  <div>
    ...
    <div v-if="cfg.mode !== 'element'" class="mt-2">
      ...
      <div v-if="cfg.mode === 'container'" class="mt-2">
        <SelectorEditor :node="node" :allowPick="true" title="Container Selector" targetKey="target" />
        <div class="hint"><small>Container element must support scrollTo(top, left)</small></div>
      </div>
    </div>
  </div>
</template>
